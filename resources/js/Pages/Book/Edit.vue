<script setup>
    ...
    const form = useForm({
        ...
        cover: null, // Initial value for cover
    })

    function update() {
        console.log(form)
        // router.post(route('books.update', { book: props.book.id }), {'_method': 'put', form})
        router.post(`/books/${props.book.id}`, {
            _method: 'put',
            form: form,
        })
        router.post(route('books.update', { book: props.book.id }), {
            _method: 'put',
            ...form,
        })
    }
        ...
        <form @submit.prevent="update">
            ...
            <div class="md:col-span-6 my-4 text-right">
                <div class="inline-flex items-end">
                    <button type="submit" :disabled="form.processing" class="w-32 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-2 rounded">Update</button>
                </div>
            </div>
        </form>
